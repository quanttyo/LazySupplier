...
from gui.global_events import EVT_DV_INS, EVT_SV_INS, EVT_TREE_SEL, EVT_DV_SEL, EVT_SV_CA
...
class MainFrame(wx.Frame):
    ...
    def __init__(self):
        wx.Frame.__init__(self, None, wx.ID_ANY, title="tool", size=(1150, 750))
        MainFrame.__instance = self

        self.SetMenuBar(MainMenuBar())
        # Arrangement of left side of mainFrame, instances of ItemBrowser and MultiSwitch
        ...
        # Events actions
    def receiving_an_instance(self, evt):
    ...
    def tree_item_selected(self, event):
        self.instances['DataView'].draw(queries.nomenclature_query(**event.object))

    def item_selected(self, event):
        self.instances['SelectedItems']._add_line(event.object[0])

    def selected_view_checkall(self, event):
        self.instances['SelectedItems']._checkall(event.state)