...
class PSplitter(wx.SplitterWindow):
    def __init__(self, parent, id=-1, proportion=0.5, size=wx.DefaultSize, minimum_pane_size=20):
        """ wx.SplitterWindow with size propotion
            PSplitter(self, parent, window_id, proportion, size, minimum_pane_size)
        """
        wx.SplitterWindow.__init__(self, parent, id=id, pos=wx.DefaultPosition, size=size, style=wx.SP_LIVE_UPDATE)
        self.SetMinimumPaneSize(minimum_pane_size)
        self.proportion = proportion
        self.reset_sash()
        self.Bind(wx.EVT_SIZE, self.on_resize)
        self.Bind(wx.EVT_SPLITTER_SASH_POS_CHANGED, self.on_sash_changed, id=id)
    ...
    def SplitHorizontally(self, window1, window2):
        if self.GetParent() is None:
            return False
        return wx.SplitterWindow.SplitHorizontally(self,
                                                   window1, window2,
    ...
    def get_expected_sash_position(self):
        """ The splitter has been moved, so we need to adjust the sash by self.proportions """

        if self.GetSplitMode() == wx.SPLIT_VERTICAL:
            value = max(self.GetMinimumPaneSize(), self.GetParent().GetClientSize().width)
        else:
            value = max(self.GetMinimumPaneSize(), self.GetParent().GetClientSize().height)
    ...
    def on_resize(self, event):
        """The window size has been changed, so we need to adjust the sash by self.proportion"""
